...
function AppealsPage() {
      ...
      comments: [
        {
          ...
          author: 'Студент',
          date: '2025-01-20',
          text: 'Изначальное обращение подано',
          type: 'system'
        },
        {
          id: 2,
          author: 'Академическая комиссия',
          date: '2025-01-22',
          text: 'Обращение принято к рассмотрению. Необходима дополнительная экспертиза.',
          type: 'official'
        }
      ],
      progress: 60
      ...
      comments: [
        {
          ...
          author: 'Студент',
          date: '2025-01-15',
          text: 'Обращение подано с приложением справки',
          type: 'system'
        },
        {
          id: 2,
          author: 'Деканат',
          date: '2025-01-17',
          text: 'Документы проверены, справка действительна',
          type: 'official'
        },
        {
          id: 3,
          author: 'Деканат',
          date: '2025-01-18',
          text: 'Пересдача одобрена и назначена',
          type: 'official'
        }
      ],
      progress: 100
      ...
      comments: [
        {
          ...
          author: 'Студент',
          date: '2025-01-10',
          text: 'Проблема с доступом к системе',
          type: 'system'
        },
        {
          id: 2,
          author: 'Техподдержка',
          date: '2025-01-11',
          text: 'Обнаружена техническая проблема, работаем над устранением',
          type: 'official'
        },
        {
          id: 3,
          author: 'Техподдержка',
          date: '2025-01-12',
          text: 'Проблема устранена, доступ восстановлен',
          type: 'official'
        }
      ],
      progress: 100
  ...
  // Функция фильтрации апелляций
  const filteredAppeals = myAppeals.filter(appeal => {
    const matchesStatus = filterStatus === 'all' || appeal.status === filterStatus;
    const matchesSearch = appeal.category.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         appeal.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         appeal.appeals_id.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const addComment = () => {
    ...
    const updatedAppeals = myAppeals.map(appeal => {
      if (appeal.id === selectedAppeal.id) {
        const newCommentObj = {
          id: appeal.comments.length + 1,
          author: 'Студент',
        ...
        return {
          ...appeal,
          comments: [...appeal.comments, newCommentObj],
    ...
    setNewComment('');
    setSelectedAppeal(updatedAppeals.find(a => a.id === selectedAppeal.id));
  };
